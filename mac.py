...
import math
import struct
from .crypto import aesDecrypt, aesEncrypt
...
class MACPayload(object):
    ...
    def encode(self):
        ...
        """
        data = self.fhdr.encode() + struct.pack('B', self.fport) + self.frmpayload
        return data
...
class JoinRequestMessage(MACMessage):
    ...
    def checkMIC(self, appkey):
        ...
        Args:
            appkey (bytes): The application key.
        ...
        aesdata = aesEncrypt(appkey, data, mode='CMAC')
...
class MACDataMessage(MACMessage):
    ...
    def encrypt(self, key, dir):
        ...
        Args:
            key (bytes): AES encryption key - device NwkSKey or AppSkey
        ...
        for i in range(k):
            ...
            S = aesEncrypt(key, Ai)
            # print("frmpayload:", self.payload.frmpayload)
            # print("type check\nfrmpayload:{}\nS:{}".format(
            #     type(self.payload.frmpayload[i]),
            #     type(S[0])))
            for index in range(plen if plen < 16 else 16):
                pld[i+index] = self.payload.frmpayload[i+index] ^ S[index]

        self.payload.frmpayload = bytes(pld)
...
class MACDataUplinkMessage(MACDataMessage):
    ...
    def decode(cls, mhdr, data):
        ...
        payload = MACPayload.decode(data[1:-4])
    ...
    def checkMIC(self, key):
        ...
        data = B0 + msg
        aesdata = aesEncrypt(key, data, mode='CMAC')
        mic = struct.unpack('<L', aesdata[:4])[0]
    ...
    def encode(self, key):
        ...
        data = B0 + msg
        # Create the MIC over the entire message
        self.mic = aesEncrypt(key, data, mode='CMAC')[:4]
        msg += self.mic
        return msg
...
class MACDataDownlinkMessage(MACDataMessage):
    ...
    def encode(self):
        ...
        data = B0 + msg
        # Create the MIC over the entire message
        self.mic = aesEncrypt(self.key, data, mode='CMAC')[0:4]
        msg += self.mic
        return msg
...
class LinkADRReq(MACCommand):
    ...
    def encode(self):
        ...
        data = struct.pack('<BBHB', self.cid,
                           datarate_txpower, self.chmask, redundancy)
        return data
